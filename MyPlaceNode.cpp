...
MyPlaceNode::MyPlaceNode(MapNode*           mapNode,
...
MyPlaceNode::init()
{
	...
	m_pMatrixTransform->addChild(_geode);

	getAttachPoint()->addChild(m_pMatrixTransform);

	// for clamping and occlusion culling    
	//OE_WARN << LC << "MyPlaceNode::applyStyle: " << _style.getConfig().toJSON(true) << std::endl;
	applyStyle(_style);
...
void
MyPlaceNode::setText(const std::string& text)
{
	if (!_dynamic)
	{
...
void
MyPlaceNode::setStyle(const Style& style)
{
...
void
MyPlaceNode::setIconImage(osg::Image* image)
{
...
void
MyPlaceNode::setAnnotationData(AnnotationData* data)
{
...
void
MyPlaceNode::setDynamic(bool value)
{
...
OSGEARTH_REGISTER_ANNOTATION(place, osgEarth::Annotation::MyPlaceNode);
...
void MyPlaceNode::RotateHeading(double dValue)
{
	double dPI = 3.1415926;
	osg::Matrix matrixRotate;
	matrixRotate.makeRotate(dValue * dPI / 180.0, osg::Vec3f(0.0, 0.0, 1.0));
	m_pMatrixTransform->setMatrix(osg::Matrix());
	m_pMatrixTransform->postMult(matrixRotate);
}